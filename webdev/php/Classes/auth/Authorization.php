...
class Authorization {
	...
	public static function userHasPermission($id, $node) {
		...
		while ($row = $result->fetch_row()) {
			...
			if($perm->match($node)) return true;
		}
		return false;
	}
	...
	public function hasPermission($node) {
		foreach($this->permissionList as $permission) {
			if($permission->hasPermission($node)) return true;
		}
		return false;
	}
	...
	public function addPermission($node, $enabled) {
		if($this->permissionExists($node)) return false;
	...
	private function permissionExists($node) {
		foreach($this->permissionList as $permission) {
			if($permission->match($node)) return true;
		}
		return false;
	}
	...
	public function revokePermission($node) {
		...
		foreach($this->permissionList as $permID => $permission) {
			if(!$permission->match($node)) continue;
	...
	public function __toString() {
		$result = '';
		foreach($this->permissionList as $key) $result .= (string)$key . "<br />";
		return $result;
	}
